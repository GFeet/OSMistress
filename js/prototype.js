...
function registerMistressMeters() {
  let mistressMeters = {
    ...
    onTick: function() {
      addMeterValue(0, "My First Meter", -1);
    }
  };
...
function addMetersToCard(cardPayload) {
  cardPayload.mistress.meters.forEach(function (meter){
    ...
    if(meter.useColors) {
      var classToAdd = "bg-success";
      if(meter.value < 60) classToAdd = "bg-warning";
      if(meter.value < 20) classToAdd = "bg-danger";

      $("#mistress-meter-clone > .progress > .progress-bar").removeClass("bg-success");
      $("#mistress-meter-clone > .progress > .progress-bar").removeClass("bg-warning");
      $("#mistress-meter-clone > .progress > .progress-bar").removeClass("bg-danger");
      $("#mistress-meter-clone > .progress > .progress-bar").addClass(classToAdd);

      $("#mistress-meter-clone").removeClass("bg-success");
      $("#mistress-meter-clone").removeClass("bg-warning");
      $("#mistress-meter-clone").removeClass("bg-danger");
      $("#mistress-meter-clone").addClass(classToAdd);
    }
...
function addMeterValue(charId, meterName, value) {
  var char = getCharacterReferenceById(charId);
  if(char === null) return;
  for(var index in char.meters)
  {
    var meter = char.meters[index];
...
function startCoreLoop() {
  coreLoop = setInterval(OnLoop, 1000);
}

function OnLoop() {
  for(var index in gameData.characters) {
    var character = gameData.characters[index];
    renderMistressCard(character.id);
  }

  triggerModsCallback("onTick");
}
